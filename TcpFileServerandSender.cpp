    ...
    : QWidget(parent), sender(new TcpFileSender(this)), receiver(new TcpFileServer(this))
{
    ...
    connect(sender->getTcpClient(), &QTcpSocket::disconnected, this, &TcpFileServerandSender::handleClientDisconnected);
}
...
void TcpFileServerandSender::handleClientDisconnected()
{
    QMessageBox::warning(this, "連線中斷", "客戶端已斷開連線，伺服器將繼續運行。");
}
